...
export const AI_PROVIDER_ADAPTERS: Record<AiProviderId, ProviderAdapter> = {
  ...
  opencode: {
    meta: metaOf('opencode'),
    capabilities: { auth: 'api-key', vision: true },
    // Local `opencode serve` on 127.0.0.1:3456; the empty baseUrl is resolved by the opencode module itself.
    resolveEndpoint() {
      return { protocol: 'opencode-app-server', baseUrl: '' }
    },
  },
}
